...
- `src/components/admin/DogStatusAction.tsx` — lines 13, 24, 34 (the `"not_available"` string in both the `setStatus` calls and the conditional render). Additionally: add an `if (updateError) { ...surface it, do not proceed to log... }` guard before the `activity_logs` insert (currently absent — this is the file that logs false successes). **Live-testing during implementation surfaced a second, non-obvious failure mode this guard alone does not catch:** when Postgres RLS's `USING` clause silently excludes a row from an `UPDATE` (e.g. the caller's admin role was revoked between page load and the click), Supabase returns `error: null` with zero rows affected — not an error — so a bare `if (updateError)` check would still let the false "success" log through in exactly that case. The actual fix adds `.select("id")` to the update call and checks `updatedRows.length === 0` as a second failure condition alongside `updateError`. This was confirmed to matter with a real reproduction: an admin session had its role revoked mid-session (via direct DB access, simulating a race/revocation), the update returned HTTP 200 with an empty array and no error, and — before this second check was added — would have logged a false success; after adding it, the UI correctly shows "A módosítás nem történt meg (nincs jogosultságod ehhez a kutyához)" and zero `activity_logs` rows are written.
...
**Corrected mechanic (found while implementing, worth recording precisely):** the project's Supabase email templates already use `{{ .ConfirmationURL }}` (confirmed by reading `mailer_templates_recovery_content`/`mailer_templates_confirmation_content` via the Management API), which is Supabase's **implicit-flow** link style — clicking it redirects to `redirectTo` with the session tokens in the URL **fragment** (`#access_token=...&refresh_token=...&type=recovery`), not a `?code=` query string. Fragments are never sent to the server, so a server-side Route Handler (the originally-planned `/auth/callback/route.ts` using `exchangeCodeForSession`) **cannot** read them — that pattern only applies to Supabase's newer PKCE/`token_hash` template style, which this project doesn't use, and switching to it would mean editing the mailer templates too (a larger, unnecessary change for this fix). The correct, smaller-diff mechanic for the templates as they exist today: point `redirectTo` directly at the destination page, and let the Supabase **browser** client's default `detectSessionInUrl: true` behavior (already the default for `createBrowserClient` from `@supabase/ssr`) auto-parse the fragment and establish the session client-side — no server exchange step needed.
...
3. New page `src/app/jelszo-visszaallitas/uj-jelszo/page.tsx` (set-new-password step) — a client component that listens for `supabase.auth.onAuthStateChange` and specifically the `PASSWORD_RECOVERY` event (fired once the browser client parses the URL fragment) before showing the form; calls `supabase.auth.updateUser({ password })` once a recovery session exists; on success, redirects to `/bejelentkezes` with a success message.

**Affected files:** `src/app/bejelentkezes/page.tsx` (add link), 2 new pages as listed above. (Task 1.7's signup/partner-confirmation landing page follows the identical client-side pattern — described there — rather than sharing a server route with this task.)
